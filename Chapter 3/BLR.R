...
colours <- c("#26316f", "#cacbff", "#e5bd0a")

ggpairs(pairsdata, aes(colour = species)) +
  scale_fill_manual(values = colours) +
  scale_color_manual(values = colours) +
  theme(panel.background = element_rect(fill = "#f5f5ff"),
        strip.background = element_rect(fill = "#26316f"),
        strip.text = element_text(colour = "#f5f5ff"))
...
ggplot(df.logharmonic, aes(x = `Number of samples`, y = Evidence, fill = "Evidence")) +
  ...
  scale_fill_manual(values = "#cacbff") +
  theme_minimal() +
  ...
  geom_abline(intercept = log_analytic, slope = 0, col = "#26316f")
...
##### Power posterior - Systematic analysis with fixed temps #####

y <- as.vector(y)

nruns <- 100
niter <- c(100, 1000, 10000, 100000)
powerposterior1 <- matrix(nrow = nruns, ncol = length(niter))
...
for (i in 1:nruns){
  for (j in 1:length(niter)){
    ...
    powerposterior1[i, j] <- logevcomplex
    cat(sprintf("Completed Run %d, Iteration Level %d\n", i, niter[j]))
  }
}

colnames(powerposterior1) <- c("100", "1000", "10000", "100000")

df.powerposterior1 <- as.data.frame(powerposterior1)
df.powerposterior1 <- pivot_longer(df.powerposterior1, cols = everything(), names_to = "Number of samples", values_to = "Evidence")

ggplot(df.powerposterior1, aes(x = `Number of samples`, y = Evidence, fill = "Evidence")) +
  geom_boxplot() +
  ylab("Log evidence") +
  ylim(c(-255.5, -250)) +
  stat_summary(
...
##### Power posterior - Systematic analysis with matching temps #####

y <- as.vector(y)

nruns <- 100
niter <- c(100, 1000, 10000, 100000)
powerposterior2 <- matrix(nrow = nruns, ncol = length(niter))
...
for (i in 1:nruns){
  for (j in 1:length(niter)){
    ...
    powerposterior2[i, j] <- logevcomplex
    cat(sprintf("Completed Run %d, Iteration Level %d\n", i, niter[j]))
  }
}

colnames(powerposterior2) <- c("100", "1000", "10000", "100000")

palatinate <- rgb(104, 36, 109, maxColorValue = 255)

df.powerposterior2 <- as.data.frame(powerposterior2)
df.powerposterior2 <- pivot_longer(df.powerposterior2, cols = everything(), names_to = "Number of samples", values_to = "Evidence")

ggplot(df.powerposterior2, aes(x = `Number of samples`, y = Evidence, fill = "Evidence")) +
  geom_boxplot() +
  ylab("Log evidence") +
  ylim(c(-255.5, -250)) +
  stat_summary(
...
# High posterior density values for calculation; mean fine here since unimodal distribution
