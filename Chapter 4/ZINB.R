...
plot_data <- data.frame(
  ...
  Source = rep(c("Original Data", "Simulated Data (Negative Binomial Model)"), each = length(y_synthetic))
)
...
plot_data <- data.frame(
  Count = c(y_synthetic, sim_counts),
  Source = rep(c("Original Data", "Simulated Data (ZINB Model)"), each = length(y_synthetic))
)
...
####### EVIDENCE ESTIMATES FOR ALL MODELS - ZIP and ZINB ########
